...
export SPACEMIT_ASR_CPUSET=${SPACEMIT_ASR_CPUSET:-${SPACEMIT_TTS_CPUSET}}
...
LOG_FILE=/tmp/phase6_multimodal_demo.log
...
if [ -n "${LAUNCH_TTY}" ]; then
  {
    echo
    echo "[launch] phase6 is starting, warmup may take about 30-40 seconds."
  ...
  if command -v taskset >/dev/null 2>&1 && [ -n "${MULTIMODAL_CPUSET}" ]; then
    taskset -c "${MULTIMODAL_CPUSET}" \
      stdbuf -oL -eL .venv/bin/python -u -m app.main \
        --config /mnt/ssd/spacemit_project/configs/multimodal_demo.yaml \
        multimodal-demo
  else
    stdbuf -oL -eL .venv/bin/python -u -m app.main \
      --config /mnt/ssd/spacemit_project/configs/multimodal_demo.yaml \
...
if ! run_phase6; then
  if [ -n "${LAUNCH_TTY}" ]; then
    {
      echo
      echo "[launch] phase6 exited unexpectedly, tailing ${LOG_FILE}:"
      tail -n 40 "${LOG_FILE}"
      echo
    } > "${LAUNCH_TTY}" 2>/dev/null || true
  fi
