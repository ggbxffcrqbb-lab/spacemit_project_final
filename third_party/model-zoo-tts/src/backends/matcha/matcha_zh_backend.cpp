...
ErrorInfo MatchaZhBackend::initializeLanguageSpecific(const TtsConfig& config) {
    try {
        // 初始化 Jieba
        auto jieba_begin = std::chrono::high_resolution_clock::now();
        initializeJieba();
        auto jieba_end = std::chrono::high_resolution_clock::now();
        if (initTraceEnabled()) {
            std::cout << "[TTS] init_timing backend=Matcha-TTS (Chinese)"
                << " phase=jieba_init ms=" << elapsedInitMs(jieba_begin, jieba_end)
                << std::endl;
        }
        ...
        if (!internal_config.lexicon_path.empty() &&
            ...
            auto lexicon_begin = std::chrono::high_resolution_clock::now();
            loadLexicon(internal_config.lexicon_path);
            auto lexicon_end = std::chrono::high_resolution_clock::now();
            if (initTraceEnabled()) {
                std::cout << "[TTS] init_timing backend=Matcha-TTS (Chinese)"
                    << " phase=lexicon_load ms=" << elapsedInitMs(lexicon_begin, lexicon_end)
                    << std::endl;
            }
        } else {
