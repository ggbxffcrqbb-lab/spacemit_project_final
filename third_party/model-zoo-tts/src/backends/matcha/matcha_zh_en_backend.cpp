...
bool initTraceEnabled() {
    const char* value = std::getenv("SPACEMIT_TTS_TRACE");
    if (value == nullptr) {
        return false;
    }
...
int64_t elapsedInitMs(std::chrono::high_resolution_clock::time_point begin,
...
ErrorInfo MatchaZhEnBackend::initializeLanguageSpecific(const TtsConfig& config) {
    try {
        // 检查 espeak-ng
        auto espeak_begin = std::chrono::high_resolution_clock::now();
        ...
        auto espeak_end = std::chrono::high_resolution_clock::now();
        if (initTraceEnabled()) {
            std::cout << "[TTS] init_timing backend=Matcha-TTS (Chinese-English)"
                << " phase=espeak_check ms=" << elapsedInitMs(espeak_begin, espeak_end)
                << std::endl;
        }
        espeak_initialized_ = true;

        // 初始化 cpp-pinyin
        auto pinyin_begin = std::chrono::high_resolution_clock::now();
        initializePinyin();
        auto pinyin_end = std::chrono::high_resolution_clock::now();
        if (initTraceEnabled()) {
            std::cout << "[TTS] init_timing backend=Matcha-TTS (Chinese-English)"
                << " phase=pinyin_init ms=" << elapsedInitMs(pinyin_begin, pinyin_end)
                << std::endl;
        }
